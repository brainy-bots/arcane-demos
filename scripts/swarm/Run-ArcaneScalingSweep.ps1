...
.DESCRIPTION
    Starts N Arcane cluster servers (arcane-cluster-demo with demo physics), arcane-manager
    for round-robin join, then runs arcane-swarm --backend arcane against the manager.
    Each cluster persists entity state to SpacetimeDB via set_entities (batch, low frequency).
    Prerequisites:
    - Redis running (default redis://127.0.0.1:6379).
    - SpacetimeDB: run "spacetime start" in another terminal. This script can build/publish
      the module from spacetimedb_demo/spacetimedb unless -NoPublish is set.

.PARAMETER NumServers
    Number of Arcane cluster servers (default 1). Players are distributed round-robin across them.

.PARAMETER PlayersTotal
    Total simulated players (default 250). Should be divisible by NumServers for even load.

.PARAMETER Duration
    Seconds to run the swarm (default 30).

.PARAMETER NoPublish
    Skip SpacetimeDB build/publish; assume "spacetime start" and module are already running.

.PARAMETER OutCsv
    CSV path for results (default: arcane_scaling_sweep.csv in script dir).

.PARAMETER SpacetimeHost
    SpacetimeDB HTTP base (default http://127.0.0.1:3000).

.PARAMETER DatabaseName
    SpacetimeDB database name (default arcane).

.PARAMETER LogDir
    Directory for cluster/manager logs (default: arcane_scaling_logs under script dir). Enables bottleneck visibility: ArcaneServerStats (tick_ms), SpacetimeDB persist times/errors.

.EXAMPLE
    .\Run-ArcaneScalingSweep.ps1 -NumServers 2 -PlayersTotal 500
.EXAMPLE
    .\Run-ArcaneScalingSweep.ps1 -NumServers 1 -PlayersTotal 250 -NoPublish
#>
param(
    [int]   $NumServers = 1,
...
# Paths
$ExeSwarm = Join-Path $RepoRoot "target\release\arcane-swarm.exe"
$ExeClusterDemo = Join-Path $RepoRoot "target\release\arcane-cluster-demo.exe"
...
# Build arcane-swarm and arcane-cluster-demo (arcane-demos) — use cmd to avoid PowerShell stderr termination
...
# Build arcane-manager (arcane repo)
...
# Persist batch size: 0 = no cap (one request per persist window); >0 = max entities per HTTP request
...
for ($i = 0; $i -lt $NumServers; $i++) {
    ...
    # Neighbor IDs so Redis replication is active (everyone sees everyone across clusters when NumServers > 1)
    ...
    $proc = Start-Process -FilePath $ExeClusterDemo -WorkingDirectory $RepoRoot -PassThru -NoNewWindow `
