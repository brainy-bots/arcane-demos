<#
.SYNOPSIS
    Find SpacetimeDB ceiling with server-physics (intended pattern): run arcane-swarm-sdk at increasing player counts.

.DESCRIPTION
    Prerequisites: run "spacetime start" in another terminal first.
    This script builds the module (with server_physics default), publishes it, builds arcane-swarm-sdk, then runs
    the swarm with --server-physics at each player count. Parses FINAL line for pass/fail (err_rate < 1%, lat_avg_ms < 200).
    Latency can be highly variable; later runs in a session may be affected by server/OS state (connections, memory).
    Use -RepeatCount 3 (or more) and -CooldownSeconds 10-30 for more stable ceiling estimates.

.PARAMETER PlayerCounts
    List of player counts to try (e.g. 100,150,200). Ignored if -FindCeiling is set.

.PARAMETER FindCeiling
    Auto-sweep from -Step up by -Step until failure or -MaxPlayers.

.PARAMETER Step
    When -FindCeiling: try N = Step, 2*Step, ... (default 250; sized to bracket ceiling ~1k–2k from server-physics call volume).

.PARAMETER MaxPlayers
    When -FindCeiling: stop when N exceeds this (default 2000).

.PARAMETER Duration
    Seconds per run (default 30). Warmup (5s) is included in arcane-swarm-sdk.

.PARAMETER NoPublish
    Skip build+publish of module (use already-published module). Run "spacetime start" and publish once manually first.

.PARAMETER OutCsv
    CSV path for results (default: spacetimedb_ceiling_sweep.csv in script dir).

.PARAMETER RepeatCount
    Run each N this many times and use median latency for pass/fail (default 1). Reduces variance; use 3+ for more stable ceiling.

.PARAMETER CooldownSeconds
    Seconds to wait between runs (default 0). Use 10–30 if you suspect server/OS resource retention from prior runs (e.g. connections, memory).

.EXAMPLE
    .\Run-SpacetimeDBCeilingSweep.ps1 -FindCeiling -Step 200 -MaxPlayers 2000
.EXAMPLE
    .\Run-SpacetimeDBCeilingSweep.ps1 -PlayerCounts 100,150,200,250 -NoPublish
#>
param(
...
$Exe = Join-Path $RepoRoot "target\release\arcane-swarm-sdk.exe"
...
$CanonicalDemoEntities = 0   # N/A for SpacetimeDB-only; no NPCs

# Resolve SpacetimeDB CLI: use PATH if available, else add Windows install dir to PATH
...
if (-not (Test-Path $Exe)) {
    ...
    cargo build -p arcane-demo --bin arcane-swarm-sdk --features swarm-sdk --release 2>&1 | Out-Host
...
# Skip N we already have results for in the CSV
$existingN = @()
$initialCeiling = $null
if (Test-Path $OutCsv) {
...
foreach ($N in $runs) {
    ...
    for ($r = 1; $r -le $RepeatCount; $r++) {
        ...
        $proc = Start-Process -FilePath $Exe -ArgumentList @(
            ...
            "--host", $SpacetimeHost,
            "--db", $DatabaseName
        ) -WorkingDirectory $RepoRoot -RedirectStandardOutput $tmpOut -RedirectStandardError $tmpErr -Wait -NoNewWindow -PassThru
